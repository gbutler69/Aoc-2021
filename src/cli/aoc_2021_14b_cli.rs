...
use aoc_2021::aoc_2021_14b::{
    determine_polymer_pairs_after_n_steps_given_rules_and_starting_polymer,
    greatest_minus_least_element_qty,
};
...
pub fn execute(path: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let final_polymer = determine_polymer_pairs_after_n_steps_given_rules_and_starting_polymer(
        rules,
        starting_polymer,
        40,
    );
    let diff_score = greatest_minus_least_element_qty(&final_polymer);
