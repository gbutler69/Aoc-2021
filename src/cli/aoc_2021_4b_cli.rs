...
use aoc_2021::{
    aoc_2021_4::Board,
    aoc_2021_4b::determine_last_winning_board_uncalled_total_last_called_number_and_score,
};
...
pub fn execute(path: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let boards = input_lines
        ...
        .collect::<Vec<_>>();
    match determine_last_winning_board_uncalled_total_last_called_number_and_score(
        boards,
        numbers_called,
    ) {
        (Some(wb), Some(ut), Some(lcn), Some(s)) => {
            println!(
                "last winning board {wb} uncalled total {ut} last called number {lcn} score {s}"
            )
        }
        _ => todo!(),
    };
