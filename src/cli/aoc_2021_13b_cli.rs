...
use aoc_2021::aoc_2021_13b::{determine_points_after_folds, Axis};
...
pub fn execute(path: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = determine_points_after_folds(points, folds)
        .into_iter()
        .map(|row| "|".to_owned() + String::from_iter(row.into_iter()).as_str() + "\n")
        .collect::<String>();
    println!("output\n{output}");
    Ok(())
}
