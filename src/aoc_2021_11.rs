...
pub fn determine_number_of_flashes_after_n_steps(
    ...
    octopi.total_flashes_for_n_steps(number_of_steps)
}
...
impl Octopi {
    fn total_flashes_for_n_steps(&mut self, number_of_steps: u16) -> u32 {
        (0..number_of_steps).map(|_| self.perform_step()).sum()
    }
